...
const sendToExperts = async (req, res) => {
    try {
        ...
        for (let i = 0; i < experts.length; i++) {
            ...
            if (expert && expert.proposals.indexOf(research_proposal_cid) !== -1) {
            ...
            if (expert) {
                ...
                const newExpert = await Expert.create(
                    { user_id: experts[i]._id, proposals: [research_proposal_cid] }
                )
                if (!newExpert.acknowledged) {
                    return res.json({ success: false, message: 'Wrong Sending Proposal to Experts.' })
                }
            }
        }
